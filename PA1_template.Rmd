...
2. No further processing needed
...
2. Print a histogram of steps per day and copy to a png file

```{r}
with(steps_per_day, hist(steps, breaks = 20, col = "wheat"))
dev.copy(png, file = "./figures/steps_per_day.png")
dev.off
```

3. Print the mean and median of 'steps' with the summary function (ANSWER: mean = 9354, median = 10400)
...
1c. plot the two variables and copy to a png file

```{r}
steps_per_5m <- aggregate(stepdata$steps, list(stepdata$interval), mean, na.rm = TRUE)
names(steps_per_5m) <- c("interval","steps")
with(steps_per_5m ,plot(x = interval, y = steps, type = "l"))
dev.copy(png, file = "./figures/steps_by_interval.png")
dev.off
...
max_interval <- subset(steps_per_5m, steps == max_steps)
print(max_interval)
```

## Imputing missing values
1. The number of missing rows can be found with the summary function. ANSWER: NA's = 2304
...
4c. Copy the plot to a png file
...
dev.copy(png, file = "./figures/steps_by_day.png")
dev.off
```
...
1c. Create a new dataframe 'stepdata3' that combines the vector 'weekday' with 'stepdata2'
...
2f. Plot a legend
2g. Copy the plot to a png file
...
dev.copy(png, file = "./figures/steps_throughout_day.png")
dev.off
```